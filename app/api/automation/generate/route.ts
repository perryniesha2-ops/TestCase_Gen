...
function extractUrlFromAction(action: string, applicationUrl: string): string | null {
    // Try to extract full URL from action text
    const fullUrlMatch = action.match(/https?:\/\/[^\s]+/);
    if (fullUrlMatch) {
        return fullUrlMatch[0];
    }

    // Try to extract path like "/settings/bank-accounts"
    ...
    const navigateMatch = action.toLowerCase().match(/navigate to|go to|visit|open/);
    if (navigateMatch) {
        // Try to find path after the phrase
        const afterPhrase = action.substring(action.toLowerCase().indexOf(navigateMatch[0]) + navigateMatch[0].length).trim();
        const pathInPhrase = afterPhrase.match(/^(https?:\/\/[^\s]+|\/[a-z0-9\/-]+)/i);
        if (pathInPhrase) {
            return pathInPhrase[0];
        }
    }
...
function postProcessSteps(steps: TestStep[], applicationUrl: string): TestStep[] {
    return steps.map((step, index) => {
        ...
        // Post-process navigation steps
        if (processedStep.action_type === 'navigate') {
            // Ensure selector is set for navigation
            if (!processedStep.selector || processedStep.selector === '') {
                processedStep.selector = 'body';
            }

            // Try to extract URL if input_value is missing
            if (!processedStep.input_value) {
                const extractedUrl = extractUrlFromAction(processedStep.action, applicationUrl);
                if (extractedUrl) {
                    processedStep.input_value = extractedUrl;
                    console.log(`[Automation] Extracted URL for step ${processedStep.step_number}: ${extractedUrl}`);
                } else {
                    console.warn(`[Automation] Navigation step ${processedStep.step_number} missing URL - check: ${processedStep.action}`);
                }
            }

            // Ensure URL assertion is set for navigation
        ...
        // Infer navigation action_type from action text if not set
        if (!processedStep.action_type && isNavigationAction(processedStep.action)) {
            ...
            if (extractedUrl) {
                processedStep.input_value = extractedUrl;
                console.log(`[Automation] Inferred navigation for step ${processedStep.step_number}`);
            }
        }
...
export async function POST(req: Request) {
    try {
        ...
        if (suiteId) {
            const { data: suiteLinks } = await supabase
                .from("test_suite_cases")
                .select("test_case_id")
                .eq("suite_id", suiteId);

            finalTestCaseIds = suiteLinks?.map(l => l.test_case_id) || [];
            console.log(`[Automation] Found ${finalTestCaseIds.length} test cases in suite`);
        }
        ...
        console.log(`[Automation] ${casesNeedingAutomation.length} test cases need automation`);
        ...
        for (const tc of casesNeedingAutomation) {
            try {
                ...
                console.log(`[Automation] Processing: ${tc.title} (${steps.length} steps)`);
...
- For EVERY step that involves navigation (navigate, go to, visit, open), you MUST set action_type="navigate" and input_value to the URL/path
- Extract URLs from the action text and put them in input_value
- Use "body" as selector for all navigation steps
- Add URL assertions for navigation steps
  ...
  "enhanced_steps": [
    {
      ...
      "assertion": {...}
                ...
                if (!parsed.enhanced_steps || !Array.isArray(parsed.enhanced_steps)) {
                    ...
                    failed.push({ id: tc.id, title: tc.title, reason: "Invalid AI response" });
                    continue;
                }

                // Post-process steps to ensure navigation steps have proper data
                const enhanced_steps = postProcessSteps(parsed.enhanced_steps, applicationUrl);

                // Log any navigation steps for debugging
                enhanced_steps.forEach(step => {
                    if (step.action_type === 'navigate') {
                        console.log(`[Automation] Navigation step ${step.step_number}:`, {
                            action: step.action,
                            selector: step.selector,
                            input_value: step.input_value,
                            has_assertion: !!step.assertion,
                        });
                    }
                });
                ...
                if (updateError) {
                    ...
                    failed.push({ id: tc.id, title: tc.title, reason: updateError.message });
                } else {
                ...
                console.error(`[Automation] Failed to enhance test case ${tc.id}:`, error);
                failed.push({
                    id: tc.id,
                    title: tc.title,
        ...
        console.log(`[Automation] Complete: ${enhanced.length} enhanced, ${failed.length} failed`);

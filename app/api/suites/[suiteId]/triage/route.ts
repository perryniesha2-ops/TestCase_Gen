...
function deriveFixSuggestions(tc: {
  ...
  if (!hasUrl) {
    suggestions.push({
      ...
      detail:
        "Include the exact page/URL or route for the first navigation step.",
    });
  }
  ...
  if (!hasData) {
    suggestions.push({
      ...
      detail:
        "Replace placeholders with real values (emails, names, numbers, boundary values).",
    });
  }

  const hasVagueExpected =
    steps.some(
      (s) => (s.expected ?? "").length < 40 && isVague(s.expected ?? ""),
  ...
  if (hasVagueExpected) {
    suggestions.push({
      ...
      detail:
        "Expected results should mention exact UI text, URL, state, or data changes.",
    });
  }

  const hasCompoundSteps = steps.some((s) =>
    (s.action ?? "").includes(" and "),
  );
  if (hasCompoundSteps) {
    suggestions.push({
      code: "SPLIT_STEPS",
...
function scoreValue(tc: TestCaseShape): number {
  ...
  const base =
    p === "critical" ? 10 : p === "high" ? 8 : p === "medium" ? 5 : 3;

  const tt = (tc.test_type ?? "").toLowerCase();
  const bump =
    tt.includes("e2e") || tt.includes("regression") || tt.includes("smoke")
      ? 1
      : 0;
...
function scoreEffort(tc: TestCaseShape): number {
  ...
  const vagueCount = steps.filter(
    (s) => isVague(s.expected ?? "") || isVague(s.action ?? ""),
  ).length;
  const compound = steps.filter((s) =>
    (s.action ?? "").includes(" and "),
  ).length;
...
function scoreConfidence(tc: TestCaseShape): number {
  ...
  const hasExpectedResult = Boolean(
    tc.expected_result && tc.expected_result.trim().length > 0,
  );
  const hasPreconditions = Boolean(
    tc.preconditions && tc.preconditions.trim().length > 0,
  );
...
function buildReasons(
  ...
  if (steps.some((s) => !(s.action ?? "").trim() || !(s.expected ?? "").trim()))
    reasons.push("Some steps are incomplete.");
  if (steps.some((s) => isVague(s.expected ?? "")))
    reasons.push("Expected results are vague; tighten assertions.");
  if (!tc.preconditions)
    reasons.push("Preconditions are missing or too brief.");
  if (!tc.expected_result) reasons.push("Overall expected result is missing.");
  if (value >= 7 && effort <= 4)
    reasons.push("High value with relatively low effort.");
  if (confidence >= 8)
    reasons.push(
      "Strong detail quality; good candidate for automation conversion.",
    );

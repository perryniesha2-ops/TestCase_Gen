  ...
  try {
    ...
    if (!["test_cases", "platform_test_cases"].includes(testTable)) {
      return NextResponse.json(
        {
          error:
            'Invalid testTable. Must be "test_cases" or "platform_test_cases"',
        },
        { status: 400 },
      );
    }
    ...
    if (!["not_run", "pass", "fail", "skip", "blocked"].includes(status)) {
      return NextResponse.json(
        {
          error:
            "Invalid status. Must be one of: not_run, pass, fail, skip, blocked",
        },
        { status: 400 },
      );
    }
    ...
    if (testCaseId && testTable) {
      ...
      if (testTable === "test_cases") {
        const { data, error } = await supabase
          .from("test_cases")
          .select(
            "id, execution_status, executed_at, executed_by, execution_notes, execution_time_minutes",
          )
          .eq("id", testCaseId)
          .eq("user_id", user.id)
          .single();

...
export function SuiteReports({ suiteId, showAllSuites = false }: SuiteReportsProps) {
  const [suiteStats, setSuiteStats] = useState<SuiteExecutionStats[]>([])
  const [testCasePerformance, setTestCasePerformance] = useState<TestCasePerformance[]>([])
...
async function fetchSuiteExecutionStats(
  ...
  async function fetchTestCasePerformance() {
    try {
      ...
      let query = supabase
        .from('test_executions')
        .select('test_case_id, execution_status, started_at, completed_at, failure_reason, created_at')
        .gte('created_at', cutoffDate)
      ...
      const testCaseIds = Array.from(new Set(executions.map(e => e.test_case_id)))
      ...
      const performance = Object.entries(testCaseGroups).map(([testCaseId, execs]) => {
        const totalExecs = execs.length
        const passedExecs = execs.filter(e => e.execution_status === 'passed').length
        const failedExecs = execs.filter(e => e.execution_status === 'failed')
        
        const passRate = totalExecs > 0 ? (passedExecs / totalExecs) * 100 : 0
        
        // Calculate average execution time
        const completedExecs = execs.filter(e => e.started_at && e.completed_at)
        const avgTime = completedExecs.length > 0
          ? completedExecs.reduce((acc, exec) => {
              const start = new Date(exec.started_at!).getTime()
              const end = new Date(exec.completed_at!).getTime()
              return acc + (end - start)
            }, 0) / completedExecs.length / 1000 / 60 // minutes
          : 0

        // Find most recent failure
        const sortedFailures = failedExecs.sort((a, b) => 
        ...
        return {
          ...
          failure_frequency: Math.round((failedExecs.length / totalExecs) * 100),
    ...
    try {
      ...
      executions.forEach(exec => {
        ...
        if (!dailyStats.has(date)) {
          dailyStats.set(date, { passed: 0, failed: 0, blocked: 0, skipped: 0, total: 0 })
        }
    ...
    try {
      const csvContent = [
        ...
        ...suiteStats.map(suite => 
          `"${suite.suite_name}",${suite.total_tests},${suite.avg_pass_rate}%,${suite.avg_execution_time},"${suite.last_execution ? new Date(suite.last_execution).toLocaleString() : 'Never'}"`
        )
      ].join('\n')
  ...
  const hasAnyData = suiteStats.length > 0 || testCasePerformance.length > 0 || executionTrends.length > 0
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {suiteStats.reduce((acc, suite) => acc + suite.execution_count, 0)}
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">
                    Across {suiteStats.length} test suite{suiteStats.length !== 1 ? 's' : ''}
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold text-green-600">
                    {suiteStats.length > 0 
                      ? Math.round(suiteStats.reduce((acc, suite) => acc + suite.avg_pass_rate, 0) / suiteStats.length)
                      : 0
                    }%
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {suiteStats.length > 0 
                      ? (suiteStats.reduce((acc, suite) => acc + suite.avg_execution_time, 0) / suiteStats.length).toFixed(1)
                      : 0
                    }m
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Performance metrics for test suites in the selected time period
                </CardDescription>
              </CardHeader>
              <CardContent>
                {suiteStats.length === 0 ? (
                    ...
                    <TableBody>
                      {suiteStats.map((suite) => (
                        <TableRow key={suite.suite_id}>
                          <TableCell className="font-medium">{suite.suite_name}</TableCell>
                          <TableCell>
                          ...
                          <TableCell>
                            <div className="flex items-center gap-2">
                              <span className={`font-medium ${
                                suite.avg_pass_rate >= 80 ? 'text-green-600' :
                                suite.avg_pass_rate >= 60 ? 'text-yellow-600' : 'text-red-600'
                              }`}>
                                {suite.avg_pass_rate}%
                              </span>
                            </div>
                          ...
                          <TableCell className="text-muted-foreground">
                            {suite.last_execution 
                              ? new Date(suite.last_execution).toLocaleDateString()
                              : 'Never'
                            }
                          </TableCell>
                        </TableRow>
            ...
            <Card>
              ...
              <CardContent>
                {testCasePerformance.length === 0 ? (
                  <div className="text-center py-8 text-muted-foreground">
                    No test case execution data available for the selected period
                  </div>
                ) : (
                  <Table>
                    <TableHeader>
                    ...
                    <TableBody>
                      {testCasePerformance.slice(0, 20).map((test) => (
                        <TableRow key={test.test_case_id}>
                          <TableCell>
                            <div>
                              <div className="font-medium">{test.test_title}</div>
                          ...
                          <TableCell>
                            <span className={`font-medium ${
                              test.pass_rate >= 80 ? 'text-green-600' :
                              test.pass_rate >= 60 ? 'text-yellow-600' : 'text-red-600'
                            }`}>
                              {test.pass_rate}%
                            </span>
                          </TableCell>
                          <TableCell>
                            <span className={`font-medium ${
                              test.failure_frequency <= 10 ? 'text-green-600' :
                              test.failure_frequency <= 25 ? 'text-yellow-600' : 'text-red-600'
                            }`}>
                          ...
                          <TableCell className="text-muted-foreground">
                            {test.last_failure_date 
                              ? new Date(test.last_failure_date).toLocaleDateString()
                              : 'None'
                            }
                          </TableCell>
                        </TableRow>
              ...
              <CardContent>
                {executionTrends.length === 0 ? (
                  <div className="text-center py-12">
                    <BarChart3 className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
                    <p className="text-muted-foreground">No execution data for the selected period</p>
                  </div>
                ) : (
                              ...
                              <>
                                {trend.passed > 0 && (
                                  <div 
                                    ...
                                    style={{ width: `${(trend.passed / trend.total) * 100}%` }}
                                  >
                                    {trend.passed}
                                  </div>
                                )}
                                ...
                                {trend.blocked > 0 && (
                                  <div 
                                    ...
                                    style={{ width: `${(trend.blocked / trend.total) * 100}%` }}
                                  >
                                    {trend.blocked}
                                  </div>
                                )}
                                {trend.skipped > 0 && (
                                  <div 
                                    ...
                                    style={{ width: `${(trend.skipped / trend.total) * 100}%` }}
                                  >
                                    {trend.skipped}
                                  </div>
                                )}

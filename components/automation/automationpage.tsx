      ...
      async (overrideAutomationAdded?: boolean) => {
        ...
        try {
          ...
          const cases = (suiteCases || []).map((sc: any) => {
            const tc = Array.isArray(sc.test_cases) ? sc.test_cases[0] : sc.test_cases;

            const hasSteps = Array.isArray(tc?.test_steps) && tc.test_steps.length > 0;
            const hasAutomationData =
                tc?.automation_metadata && Object.keys(tc.automation_metadata).length > 0;
          ...
          const automationStats = {
            ...
            ready: isAutomationAdded ? cases.filter((c) => c.ready_for_export).length : 0,
          };
  ...
  return (
      <div className="space-y-6">
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" onClick={() => router.push("/test-library")}>
              Back to Suites
            </Button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{automationAdded ? ready : 0}</div>
              <p className="text-xs text-muted-foreground">
                {automationAdded ? `${automationPercentage}% ready` : "Add automation to calculate readiness"}
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {automationAdded ? suite.automation_stats?.with_automation || 0 : 0}
              </div>
              <p className="text-xs text-muted-foreground">
                {automationAdded ? "Enhanced with selectors" : "Generated after automation is added"}
              </p>
            </CardContent>
          </Card>
          ...
          <TabsContent value="overview" className="space-y-6">
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {/* Export button visible only after Add Automation */}
                    {automationAdded && (
                        <ExportPlaywrightButton suiteId={suite.id} suiteName={suite.name} />
                    )}
                  </div>
                </div>
              </CardHeader>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Tests included in this automation suite</CardDescription>
              </CardHeader>
              <CardContent>
                {testCases.length === 0 ? (
              ...
              <CardContent className="space-y-6">
                <div className="flex items-center justify-center py-8 border-2 border-dashed rounded-lg">
                  <div className="text-center space-y-4">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold mb-2">Export Suite: {suite.name}</h3>
                    ...
                    {!automationAdded ? (
                        <div className="space-y-3">
                          <p className="text-sm text-muted-foreground">
                            Click “Add Automation” to generate readiness and enable export.
                          </p>
                          <AddAutomationButton
                              suiteId={suite.id}
                              applicationUrl="https://app.example.com"
                ...
                <div>
                  ...
                  <ol className="list-decimal list-inside space-y-2 text-sm text-muted-foreground">
                    ...
                    <li>
                      Run{" "}
                      <code className="px-1.5 py-0.5 rounded bg-muted">pnpm install</code>{" "}
                      and{" "}

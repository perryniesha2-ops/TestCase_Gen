...
import type { Requirement, TestCase, RequirementTestCase } from "@/types/requirements"
...
export function LinkTestCasesDialog({
  ...
  async function fetchAllTestCases() {
    try {
      ...
      const { data, error } = await supabase
        .from('test_cases')
        .select("id, title, test_type, priority, status, created_at, updated_at")
        .eq('user_id', user.id)
        .order('title')
...
const someSelected =
  ...
  try {
    ...
    toast.success(`Linked ${ids.length} test case${ids.length > 1 ? "s" : ""}`)
    await fetchLinkedTestCases()
    onLinked()
        ...
        <DialogHeader className="sticky top-0 z-10 bg-background px-6 py-4 border-b">
          <div className="flex items-start justify-between gap-4">
            <div className="space-y-1">
              ...
              <DialogDescription>
                Manage test case links for: <span className="font-medium">{requirement.title}</span>
              </DialogDescription>
            </div>
        ...
        return (
          ...
          >
            ...
            <div className="flex items-start gap-3 flex-1">
              <Checkbox
                checked={isSelected}
                onCheckedChange={(v) => toggleSelected(testCase.id, Boolean(v))}
                disabled={loading}
                className="mt-1"
              />
